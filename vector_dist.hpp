...
////////////////////////////////////////////////////////////////
	 ...
	 *
	 * \param out output
	 * \param time (we can append the number at the end of the file_name)
	 * \param opt VTK_WRITER, CSV_WRITER, it is also possible to choose the format for  VTK
	 *            FORMAT_BINARY. (the default is ASCII format)
	 *
	 * \return if the file has been written correctly
	 ...
	 */
	//add by zhoulei
	inline bool write_time(std::string out, St time, int opt = VTK_WRITER)
	{
		if ((opt & 0x0FFF0000) == CSV_WRITER)
		{
			// CSVWriter test
			...
			std::string output = std::to_string(out + "_" + std::to_string(v_cl.getProcessUnitID()) + "_" + std::to_string(time) + std::to_string(".csv"));
		...
		else
		{
			...
			std::string output = std::to_string(out + "_" + std::to_string(v_cl.getProcessUnitID()) + "_" + std::to_string(time) + std::to_string(".vtk"));
